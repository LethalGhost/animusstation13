...
/mob/living/say(var/message)
	...
	if (area)
		areaname = area.name
	if (area && areaname)
		if (!src.sdisabilities & 1 || !src.disabilities & 1)
			message = dd_replacetext(message, "%м", areaname)
			message = dd_replacetext(message, "%p", areaname)
	// :downs:
	if (brainloss >= 60)
		...
		message = dd_replacetext(message, "ИИ", "ЫЫ")
		message = dd_replacetext(message, "капитан", " копетан ")
		message = dd_replacetext(message, " я ", " йа ")
		message = dd_replacetext(message, " ты ", " ыы ")
		message = dd_replacetext(message, "помогите", "пмагыте")
		message = dd_replacetext(message, "гриф", "гырф")
		message = dd_replacetext(message, "инженер", "ынжунер")
		message = dd_replacetext(message, "хос", "хорс")
		message = dd_replacetext(message, "медик", "медок")
		message = dd_replacetext(message, "шаттл", "шоттл")
		message = dd_replacetext(message, "СМО", "ЧМОЩ")
		if(prob(50))
