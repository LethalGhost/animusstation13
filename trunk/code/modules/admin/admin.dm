...
/obj/admins/Topic(href, href_list)
	...
	if(href_list["call_shuttle"])
		if (src.rank in list("Badmin", "Tyrant"))
...
//		var/list/alljobs = get_all_jobs()
	...
	if (href_list["removejobban"])
		if ((src.rank in list("Administrator", "Badmin", "Tyrant"  )))
...
*/
	...
	if (href_list["makeai"]) //Yes, im fucking lazy, so what? it works ... hopefully
		if ((src.rank in list( "Tyrant", "Badmin"  )))
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["prom_demot"])
		if ((src.rank in list( "Badmin", "Tyrant" )))
			...
			if(src.level == 6)
				dat += {"
				<A href='?src=\ref[src];chgadlvl=Tyrant;client4ad=\ref[C]'>Tyrant</A><BR>
	...
	if (href_list["object_list"])
		if (src.rank in list("Badmin", "Tyrant"))
			if (config.allow_admin_spawning && ((src.state == 2) || (src.rank in list("Badmin", "Tyrant"))))
				...
				for (var/dirty_path in dirty_paths)
					...
					if (!path)
						...
						removed_paths += dirty_path
					else if (ispath(path, /mob) && !(src.rank in list("Badmin", "Tyrant")))
						removed_paths += dirty_path
...
/obj/admins/proc/announce()
	...
	if (message)
		world << "\blue <b>[usr.client.stealth ? "Tyrant" : usr.key] Announces:</b>\n \t [message]"
